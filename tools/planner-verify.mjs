...
const OWNED_UPSTREAM_FILES = {
  ...
  'src/components/Layout.jsx':
    ...
    'SVAR-M5 — owns the transient bar-drag preview state and hands the RESOLVED lane height to Grid.jsx',
  ...
  'src/components/grid/Grid.jsx':
    'SVAR-M5 — reserves the RESOLVED annotation-lane height as a blank spacer under the grid header, and shifts the grid body by the same amount, so grid and chart rows share one y',
  'src/components/grid/Grid.css':
    'SVAR-M5 — the blank marker-lane spacer, and the containing block it is positioned against',
  '.gitignore':
